...
// SHA-256 round constants (first 32 bits of the fractional parts of cube roots
...
std::string Sha256Hex(const std::uint8_t* data, std::size_t len) {
    ...
    // Message length in bits, padded to a 64-byte block.
    ...
    if (len > 0) {
        std::memcpy(&padded[0], data, len);
    }
    padded[len] = static_cast<char>(0x80);
    // Big-endian 64-bit length at the end of the last block.
    for (int i = 0; i < 8; ++i) {
        padded[padded_len - 1 - i] = static_cast<char>((bit_len >> (8 * i)) & 0xFF);
    }

    std::uint32_t w[64];
    for (std::size_t offset = 0; offset < padded_len; offset += 64) {
        for (int i = 0; i < 16; ++i) {
            const std::uint8_t* p = reinterpret_cast<const std::uint8_t*>(&padded[offset]) + i * 4;
